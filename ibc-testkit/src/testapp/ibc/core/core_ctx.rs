    ...
    S: ProvableStore + Debug,
{
    ...
    fn commitment_prefix(&self) -> CommitmentPrefix {
        CommitmentPrefix::try_from(b"ibc".to_vec()).expect("empty prefix")
    }

    fn connection_counter(&self) -> Result<u64, ContextError> {
        let store = self.ibc_store.lock();
        Ok(store.conn_counter)
    }
    ...
    S: ProvableStore + Debug,
{
    ...
    fn get_proof(&self, height: IbcHeight, path: &IbcPath) -> Option<Vec<u8>> {
        ...
        store
            ...
            .map(|p| prost::Message::encode_to_vec(&p))
    }
}
    ...
    fn client_states(&self) -> Result<Vec<(ClientId, Self::AnyClientState)>, ContextError> {
        ...
        store
            ...
            .map(|client_state_path| {
                let client_state: AnyClientState = store
                    .client_state_store
    ...
    S: ProvableStore + Debug,
{
    ...
    fn store_connection_to_client(
        ...
        let mut store = self.ibc_store.lock();
        let mut conn_ids: Vec<ConnectionId> = store
            .connection_ids_store
            .get(StoreHeight::Pending, client_connection_path)
            .unwrap_or_default();
    ...
    fn delete_packet_commitment(&mut self, key: &CommitmentPath) -> Result<(), ContextError> {
        let mut store = self.ibc_store.lock();
        store.packet_commitment_store.delete(key.clone());
        Ok(())
    }
    ...
    fn delete_packet_acknowledgement(&mut self, ack_path: &AckPath) -> Result<(), ContextError> {
        let mut store = self.ibc_store.lock();
        store.packet_ack_store.delete(ack_path.clone());
        Ok(())
    }
