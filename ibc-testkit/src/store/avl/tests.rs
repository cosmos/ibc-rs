    ...
    if let Some(node) = node_ref {
        ...
        if let Some(ref left) = node.left {
            if left.key >= node.key {
                trace!("[AVL]: Left child should have a smaller key");
                return false;
            }
        ...
        if let Some(ref right) = node.right {
            if right.key <= node.key {
                trace!("[AVL]: Right child should have a bigger key");
                return false;
            }
        ...
        if node.height != std::cmp::max(left_height, right_height) + bonus_height {
            trace!("[AVL] Heights are inconsistent");
            return false;
        }
