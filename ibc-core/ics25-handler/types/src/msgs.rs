    ...
    fn try_from(any_msg: Any) -> Result<Self, Self::Error> {
        match any_msg.type_url.as_str() {
            ...
            CREATE_CLIENT_TYPE_URL => {
                ...
                let domain_msg =
                    MsgCreateClient::decode_vec(&any_msg.value).map_err(DecodingError::Protobuf)?;
                Ok(MsgEnvelope::Client(ClientMsg::CreateClient(domain_msg)))
            }
            UPDATE_CLIENT_TYPE_URL => {
                let domain_msg =
                    MsgUpdateClient::decode_vec(&any_msg.value).map_err(DecodingError::Protobuf)?;
            ...
            RECV_PACKET_TYPE_URL => {
                let domain_msg =
                    MsgRecvPacket::decode_vec(&any_msg.value).map_err(DecodingError::Protobuf)?;
            ...
            TIMEOUT_TYPE_URL => {
                let domain_msg =
                    MsgTimeout::decode_vec(&any_msg.value).map_err(DecodingError::Protobuf)?;
