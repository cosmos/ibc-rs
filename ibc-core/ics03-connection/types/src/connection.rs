...
impl ConnectionEnd {
    pub fn new(
        ...
        if state != State::Init && versions.len() != 1 {
            return Err(ConnectionError::InvalidClientState { description: "invalid state for initializing new ConnectionEnd; expected `Init` connection state and a single version".to_string() });
        }
