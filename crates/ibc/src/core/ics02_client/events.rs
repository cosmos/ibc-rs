...
const UPGRADE_CLIENT_PROPOSAL: &str = "upgrade_client_proposal";
...
/// The content of the `key` field for the attribute containing the upgrade plan name.
const KEY_UPGRADE_STORE_ATTRIBUTE_KEY: &str = "upgrade_store";

/// The content of the `key` field for the attribute containing the upgrade plan height.
const UPGRADE_PLAN_HEIGHT_ATTRIBUTE_KEY: &str = "upgrade_plan_height";

/// The content of the `key` field for the attribute containing the upgrade plan title.
...
impl From<UpgradeClientProposal> for abci::Event {
    fn from(u: UpgradeClientProposal) -> Self {
        Self {
            kind: UPGRADE_CLIENT_PROPOSAL.to_owned(),
