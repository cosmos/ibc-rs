    ...
    Ctx: ValidationContext,
{
    ...
    if current_host_time < earliest_valid_time {
        return Err(ContextError::ConnectionError(ConnectionError::NotEnoughTimeElapsed {
            current_host_time,
            earliest_valid_time,
        }));
    }
    ...
    if current_host_height < earliest_valid_height {
        return Err(ContextError::ConnectionError(ConnectionError::NotEnoughBlocksElapsed {
            current_host_height,
