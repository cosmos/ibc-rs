...
/// `TmVerifier` for it. Note that the custom verifier does need to also implement
/// the `tendermint_light_client_verifier::Verifier` trait.
///
/// In order to wire up the custom verifier, the `verify_client_message` method on
/// the `ClientStateValidation` trait must be implemented. The simplest way to do
/// this is to import and call the standalone `verify_client_message` function located
/// in the `ibc::clients::tendermint::client_state` module, passing in your custom
/// verifier type as its `verifier` parameter. The rest of the methods in the
/// `ClientStateValidation` trait can be implemented by importing and calling their
/// analogous standalone version from the `tendermint::client_state` module, unless
/// bespoke logic is desired for any of those functions.
...
/// The default verifier for IBC clients, the Tendermint light client ProdVerifier,
/// for those users who don't require custom verification logic.
